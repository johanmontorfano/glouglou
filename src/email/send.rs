...
impl Turkey {
    ...
    pub fn send_email(&self, email: Email) -> Result<Response, String> {
        ...
        let recipient_mailbox =
            format!("{} <{}>", email.to_name, email.to_email).parse::<Mailbox>();
        let cc_mailbox =
            format!("{} <{}>", email.cc_name.unwrap_or("".into()), email.cc_email.unwrap_or("".into())).parse::<Mailbox>();
        ...
        if source_mailbox.is_err() || recipient_mailbox.is_err() {
            ...
            // If parsing has been done for CC, it means that a CC has been used and will be added to the email.
            if cc_mailbox.is_ok() {
